...
## 2026-09-12 design-flash-mini-001 第五轮：卡片背面重做 + 倾斜收敛

用户反馈：背面截图「这是啥，也太丑了吧」——倾斜导致文字发糊，背面是原始字段堆叠。

### 完成项

- **倾斜收敛**：默认姿态由 `rotateX(-7deg) rotateY(-10deg)` 收敛为 `rotateX(-2.5deg) rotateY(-4.5deg)`，双面 `translateZ` 由 14px 降到 4px，透视 1180px；翻面文字恢复清晰。
- **阴影柔化**：卡片层级阴影由 34px/72px 收敛为 18px/46px；景深偏移与透明度同步降低，避免大块模糊光晕。
- **背面重做**：
  - 顶部签名徽章 + check 图标；
  - 中部身份姓名，替代原来的字段列表；
  - Owner / Created 使用 2 列小卡；
  - BAP ID 独立块 + Copy 按钮；
  - Publish Transaction 收进 `details/summary`，默认折叠，避免长 TXID 撑爆卡面；
  - 修正 `dt/dd` 不在 `dl` 内的 axe `dlitem` 违规。
- **截图**：新增 `identity-back-light-1440.png`、`identity-back-dark-1440.png`，截图总数 21。

### 验证结果

- fresh-design 0 命中；模型 28/28；token 29/29、0 自造颜色；浏览器 14/14；axe 15 次 0 violations、4–5 条 incomplete（4–6 节点）逐条人工复核；对比度 11/11；离线 3/3；截图 21 张。
- 320/390/768/960/1440 无横向溢出，完整 BAP ID 与关键 CTA 首屏可见；背面 axe 0 violations / 0 incomplete。

### 提交

- 子仓库：`be62487`（第五轮背面重做，PRD v0.1_20260912-123447）、`7f46ba4`（干净检出复验，PRD v0.1_20260912-123721）。

### 唯一下一步

用户复核正面立体感与背面 Proof 面板排版；如仍不满意，可按上一轮列出的线条 A/B/C/D 方向继续迭代。
