...
- 后续修复：弧线 `border` 原用 `var(--line)`（`--s2-gray-200`，亮色 `rgb(225,225,225)` 接近白），在浅色天蓝底上几乎不可见，用户反馈「完全看不到」。经两轮加深加粗，最终 `border: 3px solid var(--s2-gray-900)`（亮色 `rgb(19,19,19)` 近黑、暗色 `rgb(242,242,242)` 近白）、`opacity` 0.95→0.68 递减，亮暗均清晰。
- 验证：Babel 通过；浏览器确认 7 条弧线 3px 近黑/近白渲染；截图像素分析深色占比由 1%→2.9%。
