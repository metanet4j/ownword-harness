...
- 后续修复：弧线 `border` 原用 `var(--line)`（`--s2-gray-200`，亮色 `rgb(225,225,225)` 接近白），在浅色天蓝底上几乎不可见，用户反馈「完全看不到」。改为 `var(--line-strong)`（`--s2-gray-700`，亮色 `rgb(80,80,80)` 深灰、暗色 `rgb(175,175,175)` 浅灰），亮暗均清晰。
- 验证：Babel 通过；浏览器确认 7 条弧线，亮色 border=`rgb(80,80,80)`、暗色 `rgb(175,175,175)` 可见。
