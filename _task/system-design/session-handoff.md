...
- 页面标题排版已优化：仅调整 `.page-heading h1` 的断行、中文行距与字距，以及窄屏中文字号；不改文案、业务流程、欢迎页或用户资料。
...
本次标题排版仅检查 1440px 与 320px 中文浅色编辑页，以及窄屏较长身份标题；截图复核通过。编辑标题完整一行，身份标题按逗号分两行，无横向溢出或运行错误。证据为 `evidence/type-polish-*`；未跑整套回归，未验证英文、深色及其他页面组合。预览曾缓存旧 CSS，可强制刷新。以下均为此前批次结果。
...
用户预览复核页面标题，重点看窄屏中文断句与行距，再按反馈调整。资产仍为 `needs-review`，不能标记整体事项完成。链上记录键盘访问缺口已补齐，3D 卡片拖动效果保留。
