...
后续验证遵循 [任务指南的原型验证范围](AGENTS.md#原型验证范围)，不沿用下列历史专项规模作为每轮必跑要求。本次规则更新仅检查文档差异，未运行原型测试。
...
用户预览复核长内容卡面摘要与完整资料展开，再按反馈调整。资产仍为 `needs-review`，不能标记整体事项完成。链上记录键盘访问缺口已补齐，3D 卡片拖动效果保留。
