...
- astra 子仓库工作区干净，最近提交 `3192c8e`（PRD v0.1_20260911-123602）：按用户要求移除公开身份卡旁的视角控件
  （`View chain record` / `Pause rotation` / `Reset view` / `Viewing angle` 与 reduced-motion 提示），保留自动旋转与指针拖拽翻面；
  词典 155 → 148 键，浏览器断言 451 → 450，五套脚本全绿（74 / 4 / 66 / 450 / 4，axe 0 violations）。
